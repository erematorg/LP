       ...
       match thinker_state {
           ...
           ActionState::Cancelled => {
               ...
               if let Some(current) = &mut thinker.current_action {
                   ...
                   let state = action_states.get_mut(current.0.0).expect("Missing current action").clone();
                   match state {
                       ActionState::Success | ActionState::Failure => {
                       ...
                       _ => {
                           let mut state = action_states.get_mut(current.0.0).expect("Missing action");
           ...
           ActionState::Executing => {
               ...
               if let Some(choice) = thinker.picker.pick(&thinker.choices, &scores) {
                   ...
                   let mut curr_action_state = action_states.get_mut(action_ent.0).expect("Missing current action");
...
fn should_schedule_action(
   ...
   if thinker.scheduled_actions.is_empty() {
       ...
       let curr_action_state = states.get_mut(action_ent.0).expect("Missing current action");
...
fn exec_picked_action(
   ...
   if let Some((action_ent, ActionBuilderWrapper(current_id, _))) = &mut thinker.current_action {
       let mut curr_action_state = states.get_mut(action_ent.0).expect("Missing current action");
       let previous_done = matches!(
           *curr_action_state,
           ActionState::Success | ActionState::Failure
       );
