use bevy::prelude::*;
use energy::thermodynamics::thermal::{Temperature, ThermalConductivity, heat_conduction};
...
struct InputState { hot_mode: bool }

#[derive(Component)]
struct GridCell { x: usize, y: usize }
...
fn handle_input(
   ...
   if keyboard_input.just_pressed(KeyCode::Space) {
       ...
       if let Ok(mut text) = text_query.get_single_mut() {
           *text = Text(format!("Click: Set heat source\nSpace: Toggle hot/cold\nCurrent: {}", 
   ...
   if mouse_input.just_pressed(MouseButton::Left) {
       if let Some(cursor_position) = windows.single().cursor_position() {
           if let Ok(world_position) = camera_q.single().0.viewport_to_world_2d(camera_q.single().1, cursor_position) {
               for (_cell, mut temp, transform) in grid_cells.iter_mut() {
                   ...
                   if world_position.x >= cell_pos.x - half_size && world_position.x <= cell_pos.x + half_size &&
                      world_position.y >= cell_pos.y - half_size && world_position.y <= cell_pos.y + half_size {
