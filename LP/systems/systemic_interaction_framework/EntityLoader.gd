...
func load_entity_data():
	...
	if entity_registry.size() > 0:
		...
		print("Error loading JSON or empty data in 'entities.json'")
...
func update_entities_json() -> bool:
	...
	if dir:
		...
		print("Error opening directory: ", data_path)
		return false

	# Only update the JSON if there are changes
	...
	if current_entities_data != entities_data:
		...
		if file:
			...
			print("Entities JSON file updated successfully.")
			return true
		else:
			print("Error writing to JSON file: 'entities.json'")
			return false
...
func get_entity_data(type: String, category: String) -> Dictionary:
	if not entity_registry.has(category):
		print("Category not found in entity registry: ", category)
		return {}
	...
	print("Type not found in category: ", type)
	return {}

# Load all entities from the registry
func load_entities():
	if entity_registry.size() == 0:
		print("Error: No entities to load from the registry.")
		return
