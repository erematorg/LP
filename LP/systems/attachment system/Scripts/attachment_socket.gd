...
func assign_new_limb(part : EntityPart):
	if not part or enabled or part.entity_type != accepted_type:
...
func update_ik_type():
	if not creature_creator:
		return
	creature_creator.attachment_tracker.update_ik_types()
...
func update_state():
	if enabled:
		...
		return
	#find_closest_bone()
	creature_creator.find_close_entity(self, my_entity)
	if my_entity:
